...
import { cacheDel, cacheAside, CACHE_TTL } from "../../utils/cache";
...
export const getOffers = asyncHandler(async (_req: Request, res: Response) => {
  const offers = await cacheAside(OFFERS_KEY, CACHE_TTL.OFFERS, async () => {
    const data = await Offer.find().lean();
    return data.map(o => ({ ...o, productCount: o.productIds?.length ?? 0 }));
  });
